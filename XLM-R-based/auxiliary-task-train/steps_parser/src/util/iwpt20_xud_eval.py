...
def load_conllu(file,treebank_type):
    ...
    while True:
        ...
        if not line:
            ...
            #    raise UDError("There are multiple roots in a sentence: {}".format(word.columns[FORM]))
...
def evaluate(gold_ud, system_ud):
    ...
    def enhanced_alignment_score(alignment, setting="elas"):
        ...
        for words in alignment.matched_words:
                ...
                if setting == "eulas":
                    gold_deps, system_deps = strip_subtypes(gold_deps), strip_subtypes(system_deps)
                if setting == "euas":
                    gold_deps, system_deps = strip_labels(gold_deps), strip_labels(system_deps)
    ...
    def strip_subtypes(deps):
        processed_deps = []
        for (head, steps) in deps:
            processed_steps = [elem.split(":")[0] for elem in steps]
            processed_deps.append((head, processed_steps))
        return processed_deps

    def strip_labels(deps):
        processed_deps = []
        for (head, steps) in deps:
            processed_steps = ["dep" for elem in steps]
            processed_deps.append((head, processed_steps))
        return processed_deps
    ...
    return {
        ...
        "EULAS": enhanced_alignment_score(alignment, setting="eulas"),
        "EUAS": enhanced_alignment_score(alignment, setting="euas"),
...
def evaluate_wrapper(args):
    ...
    gold_ud = load_conllu(args.gold_file, treebank_type)
    system_ud = load_conllu(args.system_file, treebank_type)

    return evaluate(gold_ud, system_ud)

def evaluate_enhanced(args):
    # Evaluate
    ...
    if not args.verbose and not args.counts:
        ...
        print("-----------+-----------+-----------+-----------+-----------")
        for metric in["Tokens", "Sentences", "Words", "UPOS", "XPOS", "UFeats", "AllTags", "Lemmas", "ELAS", "EULAS", "EUAS"]:
            if args.counts:
