...
class MultiParser(nn.Module):
    ...
    def __init__(self, embeddings_processor, outputs, post_processors=None):
        ...
        #mycode 
        #self.trde_vec_model = ft.load_fasttext_format("/mount/projekte18/codeswitch/betul/fasttext/pretrained/cc.tr.300.bin")
    ...
    def evaluate_batch(self, gold_sentences, post_process=False):
        ...
        Args:
            gold_sentences: The gold AnnotatedSentences to run the parser on.
    ...
    def _compute_logits_and_labels(self, input_sents):
        ...
        #    embeddings_new[output_id] = torch.zeros(list(embeddings[output_id].size())[0],list(embeddings[output_id].size())[1],1068).cuda()
         

        #for i,sent in enumerate(input_sents):
        #   for j,word in enumerate(sent):

        #      fasttext_vec = self.trde_vec_model[word]
       
        #      for output_id in self.outputs:
        #          embeddings_new[output_id][i][j] = torch.cat((embeddings[output_id][i][j],  torch.from_numpy(fasttext_vec).cuda()),dim=0)
        ...
        labels = dict()
        for output_id in self.outputs:
            curr_logits, curr_labels = self.outputs[output_id](embeddings[output_id], true_seq_lengths) #mycode changed embeddings with embeddings_new
