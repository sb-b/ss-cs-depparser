...
class MultiParser(nn.Module):
    ...
    def __init__(self, embeddings_processor, outputs, post_processors=None):
        ...
        #mycode
        self.seq_embed = [] #embeddings_processor
        

        self.wa_layer = [] # torch.nn.Linear(2 * 768, 768).cuda() #mycode
    ...
    def parse(self, sentence):
        ...
        self.eval()
        logits, labels = self._compute_logits_and_labels(singleton_batch, 0) #mycode i added 0 for epoch
    ...
    def evaluate_batch(self, gold_sentences, epoch, post_process=False): #mycode i added epoch
        ...
        logits, labels = self._compute_logits_and_labels(sent_batch, epoch) #mycode i added epoch
    ...
    def _compute_logits_and_labels(self, input_sents, epoch):#mycode i added epoch
        ...
        #mycode
        seq_embeddings, seq_true_seq_lengths = self.seq_embed(input_sents)

        #mycode 
 
        embeddings_gated = dict()
        for output_id in self.outputs:
            
            embeddings_gated[output_id] = torch.zeros(list(embeddings[output_id].size())[0],list(embeddings[output_id].size())[1],768).cuda()
         

        if epoch < 10000:
           for output_id in ['heads','labels']:
               gated = self.wa_layer(embeddings[output_id].cuda(),seq_embeddings['noc'].cuda())
               del embeddings[output_id]

               #mycode trying to improve the scores
            
               embeddings_gated[output_id] = gated
        ...
        labels = dict()
        for output_id in self.outputs:
            if epoch < 10000: #mycode
               curr_logits, curr_labels = self.outputs[output_id](embeddings_gated[output_id], true_seq_lengths)
            else:
