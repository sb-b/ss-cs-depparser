...
def read_arguments():
    ...
    args_.add_argument('--dataset', choices=['ud', 'ontonotes'], help='Dataset', required=True)
    args_.add_argument('--src_domain', help='source domain/language', required=True)
    args_.add_argument('--tgt_domain', help='target domain/language', required=True)
    ...
    for split in args_dict['splits']:
        ## FOR DA after sequence tagger
        if split != 'test':
            args_dict['data_paths'][split] = data_path + '_' + split + '_' + args_dict['src_domain']
        else:
            args_dict['data_paths'][split] = data_path + '_' + split + '_' + args_dict['src_domain'] #mycode changed tgt_domain to src_domain
    ...
    for split in args_dict['splits']:
        ...
        if args_dict['dataset'] == 'ontonotes':
            ...
            #mycode
            if '_' in args_dict['src_domain']:

                args_dict['alphabet_data_paths'][split] = data_path + '_' + split + '_' + \
                                                      args_dict['src_domain'].split('_')[0]            
            else:
                if split == "train":
                    print("\n\n\n SECOND IF\n\n")
                    args_dict['alphabet_data_paths'][split] = "/mount/projekte/codeswitch/betul/DCST/data/merged/tr_imstboun-ud-train-merged.conllu"
                else:
                    args_dict['alphabet_data_paths'][split] = data_path + '_' + split + '_' + args_dict['src_domain']

    args_dict['model_name'] = 'domain_' + args_dict['tgt_domain']
    ...
    args_dict['alphabet_path'] = path.join(args_dict['model_path'], 'alphabets' + '_src_domain_' + args_dict['tgt_domain'] + '/')
...
def creating_alphabets(alphabet_path, alphabet_data_paths, word_dict):
    ...
    print(extra_paths)
   # extra_paths = [train_paths, train_paths]
    alphabet_dict = {}
...
def build_model_and_optimizer(args):
    ...
    if args.freeze_sequence_taggers:
        print('Freezing sequence_taggers')
...
def in_domain_evaluation(args, datasets, model, optimizer, dev_eval_dict, test_eval_dict, epoch,
    ...
    curr_dev_eval_dict = evaluation(args, datasets['dev'], 'dev', model, args.tgt_domain, epoch, 'current_results')
    ...
    if is_best_in_domain:
        ...
        curr_test_eval_dict = evaluation(args, datasets['test'], 'test', model, args.tgt_domain, epoch, 'current_results')
    ...
    if epoch == args.num_epochs:
        ...
        for split in splits_to_write:
            if split == 'dev':
                ...
                eval_dict = None
            write_results(args, datasets[split], args.tgt_domain, split, best_model, args.tgt_domain, eval_dict)
...
def main():
    ...
    logger.info('Training INFO of in domain %s' % args.tgt_domain)
    ...
    if not args.eval_mode:
        ...
        for epoch in range(start_epoch + 1, args.num_epochs + 1):
            ...
            for batch_num, batch in enumerate(train_iter):
                ...
                if batch_num % 50 == 0:
                    log_info = 'train: %d/%d, domain: %s, total loss: %.2f, arc_loss: %.2f, arc_tag_loss: %.2f, time left: %.2fs' % \
                               (batch_num, num_batches, args.tgt_domain, total_loss / total_train_inst, total_arc_loss / total_train_inst,
            ...
            if patient >= args.schedule:
                ...
                patient = 0
            print_results(test_eval_dict['in_domain'], 'test', args.tgt_domain, 'best_results')
            print('\n')
        for split in datasets.keys():
            evaluation(args, datasets[split], split, best_model, args.tgt_domain, epoch, 'best_results')

    else:
        logger.info("Evaluating")
        epoch = start_epoch
        for split in ['train', 'dev', 'test']:
            eval_dict = evaluation(args, datasets[split], split, model, args.tgt_domain, epoch, 'best_results')
            write_results(args, datasets[split], args.tgt_domain, split, model, args.tgt_domain, eval_dict)
