...
def simplified_morp_feats(model_path, parser_path, src_domain, tgt_domain, use_unlabeled_data=True, use_labeled_data=True):
    if src_domain == tgt_domain:
        pred_paths = []
        if use_unlabeled_data:
            pred_paths = [file for file in os.listdir(parser_path) if
...
def add_lang_ids(model_path, parser_path, src_domain, tgt_domain, use_unlabeled_data=True, use_labeled_data=True):
    if src_domain == tgt_domain:
        pred_paths = []
        if use_unlabeled_data:
            pred_paths = [file for file in os.listdir(parser_path) if
...
def add_case_label(model_path, parser_path, src_domain, tgt_domain, use_unlabeled_data=True, use_labeled_data=True):
    if src_domain == tgt_domain:
        pred_paths = []
        if use_unlabeled_data:
            pred_paths = [file for file in os.listdir(parser_path) if
...
def add_morp_label(model_path, parser_path, src_domain, tgt_domain, use_unlabeled_data=True, use_labeled_data=True):
    if src_domain == tgt_domain:
        pred_paths = []
        if use_unlabeled_data:
            pred_paths = [file for file in os.listdir(parser_path) if
            ...
            for line in file:
                ...
                if len(line) == 0:
                    for idx in range(len_sent):
                        ...
                        morp_feat = lines[idx][3]

                        lines[idx].append(morp_feat)
...
def add_deprel_label(model_path, parser_path, src_domain, tgt_domain, use_unlabeled_data=True, use_labeled_data=True):
    if src_domain == tgt_domain:
        pred_paths = []
        if use_unlabeled_data:
            pred_paths = [file for file in os.listdir(parser_path) if
...
def add_dummy_task(model_path, parser_path, src_domain, tgt_domain, use_unlabeled_data=True, use_labeled_data=True):
    if src_domain == tgt_domain:
        pred_paths = []
        if use_unlabeled_data:
            pred_paths = [file for file in os.listdir(parser_path) if
    ...
    rpe_f=codecs.open('/mount/projekte18/codeswitch/betul/steps_parser_2/lince_rpt_labels.vocab', encoding='utf-8', errors='ignore')

    rpe_tags = rpe_f.readlines()

    paths = pred_paths + gold_paths
    print("Adding labels to paths: %s" % ', '.join(paths))
    root_line = ['0', ROOT, 'XX', 'O', '0', 'root', rpe_tags[random.randint(0,len(rpe_tags)-1)]]
    writing_paths = {}
    sentences = {}
    for path in paths:
        if tgt_domain in path:
            ...
            for line in file:
                ...
                if len(line) == 0:
                    for idx in range(len_sent):
                        ...
                        lines[idx].append(rpe_tags[random.randint(0,len(rpe_tags)-1)])
                        

                    if len(lines) > 0:
                        sentences_list.append(['0', ROOT, 'XX', 'O', '0', 'root', rpe_tags[random.randint(0,len(rpe_tags)-1)]])
                    for line_ in lines:
                        sentences_list.append(line_)
                    sentences_list.append([])
                    lines = []
